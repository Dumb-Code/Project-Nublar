package com.nyfaria.projectnublar.block.entity;
...
public class ProcessorBlockEntity extends BaseContainerBlockEntity implements GeoBlockEntity {
    ...
    public void tick(Level world, BlockPos pos, BlockState pState, ProcessorBlockEntity be) {
        ...
        if (be.fluidLevel > 0 && !be.input.isEmpty() && !be.testTube.isEmpty() && !be.filter.isEmpty()) {
            if (be.cookingProgress < be.getMaxProcessingTime()) {
                ...
                double dnaPercentage = 0;
                if(be.input.getItem() instanceof AmberItem){
                    dnaPercentage = be.input.getOrCreateTag().getDouble("dna_percentage");
                } else {
                    dnaPercentage = FossilsConfig.getQuality(be.input.getOrCreateTag().getString("quality")).dnaYield().get() / 100d;
                }
                stack.getOrCreateTag().putDouble("dna_percentage", NublarMath.round(dnaPercentage * be.getFilterEfficiency(),2));
                stack.getTag().putString("dino", be.input.getOrCreateTag().getString("dino"));
                be.input.shrink(1);
